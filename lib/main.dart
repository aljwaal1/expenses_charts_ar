...
class RowItem {
  ...
  static RowItem decode(String raw) {
    final p = raw.split('|||');
    return RowItem(p.isNotEmpty ? p[0] : 'عملية', p.length > 1 ? double.tryParse(p[1]) ?? 0 : 0, p.length > 2 ? p[2] == '1' : false, p.length > 3 ? p[3] : 'عام', p.length > 4 ? DateTime.tryParse(p[4]) ?? DateTime.now() : DateTime.now());
  }
}
...
class App extends StatelessWidget {
  ...
  Widget build(BuildContext context) => MaterialApp(
    ...
    theme: ThemeData(useMaterial3: true, colorScheme: ColorScheme.fromSeed(seedColor: seed), scaffoldBackgroundColor: const Color(0xFFEEF7F2), fontFamily: 'Arial'),
...
class _HomeState extends State<Home> {
  ...
  Future<void> load() async { final p = await SharedPreferences.getInstance(); final saved = p.getStringList('expenses_v3') ?? []; setState(() => rows = saved.map(RowItem.decode).toList()); }
  Future<void> save() async { final p = await SharedPreferences.getInstance(); await p.setStringList('expenses_v3', rows.map((e) => e.encode()).toList()); }
  ...
  void add() {
    ...
    if (v <= 0) return;
    setState(() { rows.insert(0, RowItem(t.isEmpty ? 'عملية' : t, v, income, category, DateTime.now())); title.clear(); amount.clear(); });
    save();
    SystemSound.play(SystemSoundType.click);
  }

  void remove(RowItem item) {
    final i = rows.indexOf(item);
    setState(() => rows.remove(item));
    save();
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: const Text('تم حذف العملية'), action: SnackBarAction(label: 'تراجع', onPressed: () { setState(() => rows.insert(i < 0 ? 0 : i, item)); save(); })));
  }

  void copyReport() {
    final text = StringBuffer()
      ..writeln('$appTitle $appVersion')
      ..writeln('الدخل: ${inc.toStringAsFixed(2)}')
      ..writeln('المصروف: ${out.toStringAsFixed(2)}')
      ..writeln('الصافي: ${net.toStringAsFixed(2)}')
      ..writeln('---');
    for (final e in rows) { text.writeln('${e.title} | ${e.income ? 'دخل' : 'مصروف'} | ${e.category} | ${e.value.toStringAsFixed(2)}'); }
